...
public class MySqlExpedienteDAO implements ExpedienteDAO {
	...
	public List<Expediente> listAllByEstado(String estado) {
		...
		try {
			cn=MySqlConexion.getConectar();
			String sql=" select codigoExpediente , codExpediente,fechaExpe,fechaExpeSalida,nomSecretaria,estadoExpe,codOficio,mensaje"
			...
			while(rs.next()) {
				...
				expediente.setMens(rs.getString(8));
